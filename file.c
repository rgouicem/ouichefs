	...
	*/
					...
					struct buffer_head *bh_result, int create)
	{
		...
		*/
		...
		ref_ct = sbi->rc_table[index->blocks[iblock]].ref_count;

		if (ref_ct > 1 || index->blocks[iblock] == 0 ) {
			if (!create)
				return 0;

			if(ref_ct > 1) {
				pr_info("copy-on-write on block %d.", index->blocks[iblock]);

				dec_ref_count(sbi->rc_table, index->blocks[iblock]);
			}
